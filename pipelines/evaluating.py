...
from .utils import inverse_affine_matrix, add_third_row
...
class AlignPrediction():
    def __init__(self, **kwargs):
        ...
        self.set_name = kwargs.get('set_name', 'train')
        self.noise_type = kwargs.get('noise_type', 'noise_type')
    ...
    def predict(self):
        ...
        data_set = AlignDatagen(self.data_dir, sample_size=self.sample_size, set_name=self.set_name, city=self.city,
                                 synth_method=self.synth_method, patch_size=self.patch_size, noise_type=self.noise_type)
        data_loader_val = DataLoader(data_set, self.batch_size, drop_last=False, num_workers=4, shuffle=True)
        ...
        iou_eval_m = JaccardIndex(task="binary").to(self.device)
        ...
        for i, val_batch in pbar_val:
            ...
            with torch.no_grad():
                ...
                iou_eval_m.update(true_mask, new_true_mask)
                ...
                iou_eval = iou_eval_m.compute().item()
                ...
                dict_for_postfix["iou_evl"] = f'{iou_eval:.4f}'
