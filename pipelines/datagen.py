...
from .utils import create_affine_matrix, warp_mask_with_affine


class AlignDatagen:
    def __init__(self, data_dir, sample_size=None, start_thres=0, end_thres=None, set_ths=50,
                 city=None, single_index=None, set_name=None, synth_method=1,
                 aug_shift=0, patch_size=None, hold_city=None, noise_type="rand", rescale_value=2000):

        self.set_ths = set_ths
        ...
        file_dir = f"{self.data_dir}/{self.city}/data.csv"
        self.df = pd.read_csv(file_dir)
        print(f"Number of images in {self.city} are: {len(self.df)}")

        if hold_city is not None:
            # self.df = self.df[self.df['city'] != hold_city].reset_index(drop=True)
            # print(f"Number of images after hold out {city} are: {len(self.df)}")
        ...
        if start_thres != 0:
            self.df = self.df[self.df[f'norm_{self.set_ths}'] >= start_thres].reset_index(drop=True)
        if end_thres is not None:
            self.df = self.df[self.df[f'norm_{self.set_ths}'] <= end_thres].reset_index(drop=True)
    ...
    def __getitem__(self, index):
        ...
        if self.aug_shift is not None and self.aug_shift != 0 and self.noise_type != "r":
            tx += np.random.randint(-self.aug_shift, self.aug_shift)
            ty += np.random.randint(-self.aug_shift, self.aug_shift)
            aug_shift_rot = round(self.aug_shift/2)
            theta += np.random.randint(-aug_shift_rot, aug_shift_rot)

        image_path = os.path.join(f"{self.data_dir}/{self.city}/{self.set_name}/images/", self.df.iloc[index]['filename'])
        label_path = os.path.join(f"{self.data_dir}/{self.city}/{self.set_name}/labels/", self.df.iloc[index]['filename'])
    ...
    def create_aug_data(self, y, affine, device):
        ...
        # # erosion (for randomly selected ones with erosion range from 1 to 8 pixels)
        # errosion_coin = torch.floor(torch.rand((B, 1, 1, 1), device=device) + self.errosion_chance)
        # kernel_size = random.choice([3, 5, 7, 9, 11, 13, 15, 17])
        # neg_img = -y[:, [0]].clone()
        # eroded = -F.max_pool2d(neg_img, kernel_size=kernel_size, stride=1, padding=kernel_size // 2)
        # y[:, [0]] = y[:, [0]] * (1 - errosion_coin) + eroded * errosion_coin
        #
        # # Update affine matrices after erosion
        # zero_mask = torch.sum(y[:, [0]].reshape(B, -1), dim=1) < 1
        # identity = torch.eye(2, 3, device=device).unsqueeze(0).expand(B, -1, -1)
        # affine[zero_mask] = identity[zero_mask]
        ...
        # y[:, [0]] = y[:, [0]] * (1 - elastic_coin) + ElastT(y[:, [0]]) * elastic_coin
        ...
        data = y * (1 - rot90_coin) + aug(y) * rot90_coin
